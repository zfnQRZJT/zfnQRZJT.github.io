...
Anyway, to summarize, we have an exact solution for \\(t(x)\\) and an easy way to compute the taylor series for \\(x(t)\\), with some scaling to get rid of the constants, and make as many things as possible \\(1\\). As in the last blog post, we would have liked something neater, but that doesn't always happen.`],
  ...
  <br>
  Obviously, the game is stacked against us. But let's say we end up losing \\(100\\) D. We want a profit, so let's bet \\(200\\) D. If we get lucky, we'll net \\(100\\) D, and if we get unlucky, we'll just bet \\(400\\) D to again hopefully net $100$ D. If we keep losing, we will repeat the process until we net \\(100\\) D or we can no longer double our bets, in which case we will reset to betting \\(100\\) D, assuming we have that much to our name. I'll call going below \\(100\\) D a death, because no more bets can occur and we are stuck forever.\
  <br>
